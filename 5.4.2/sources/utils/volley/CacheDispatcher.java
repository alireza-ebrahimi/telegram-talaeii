...
public class CacheDispatcher extends Thread {
    ...
    private static class WaitingRequestManager implements Request$NetworkRequestCompleteListener {
        ...
        private synchronized boolean maybeAddToWaitingRequests(Request<?> request) {
            ...
            synchronized (this) {
                ...
                if (this.mWaitingRequests.containsKey(cacheKey)) {
                    ...
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m14404d("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
                    }
                    z = true;
                } else {
                    ...
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m14404d("new request, sending to network %s", cacheKey);
                    }
                }
            }
            return z;
        ...
        public synchronized void onNoUsableResponseReceived(Request<?> request) {
            ...
            if (!(list == null || list.isEmpty())) {
                if (VolleyLog.DEBUG) {
                    VolleyLog.m14407v("%d waiting requests for cacheKey=%s; resend to network", Integer.valueOf(list.size()), cacheKey);
                }
                ...
                try {
                    ...
                    VolleyLog.m14405e("Couldn't add request to queue. %s", e.toString());
        ...
        public void onResponseReceived(Request<?> request, Response<?> response) {
            ...
            if (list != null) {
                if (VolleyLog.DEBUG) {
                    VolleyLog.m14407v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(list.size()), cacheKey);
                }
    ...
    public void run() {
        if (DEBUG) {
            VolleyLog.m14407v("start new dispatcher", new Object[0]);
        }
