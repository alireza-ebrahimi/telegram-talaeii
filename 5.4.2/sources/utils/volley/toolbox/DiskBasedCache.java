...
public class DiskBasedCache implements Cache {
    ...
    static class CacheHeader {
        ...
        boolean writeHeader(OutputStream outputStream) {
            try {
                ...
                VolleyLog.m14404d("%s", e.toString());
                return false;
            }
        }
    }
    ...
    private void pruneIfNeeded(int i) {
        if (this.mTotalSize + ((long) i) >= ((long) this.mMaxCacheSizeInBytes)) {
            int i2;
            if (VolleyLog.DEBUG) {
                VolleyLog.m14407v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (it.hasNext()) {
                ...
                if (getFileForKey(cacheHeader.key).delete()) {
                    ...
                    VolleyLog.m14404d("Could not delete cache entry for key=%s, filename=%s", cacheHeader.key, getFilenameForKey(cacheHeader.key));
                }
                it.remove();
                i2 = i3 + 1;
            ...
            i2 = i3;
            if (VolleyLog.DEBUG) {
                VolleyLog.m14407v("pruned %d files, %d bytes, %d ms", Integer.valueOf(i2), Long.valueOf(this.mTotalSize - j), Long.valueOf(SystemClock.elapsedRealtime() - elapsedRealtime));
            }
        }
    }
    ...
    public synchronized void clear() {
        synchronized (this) {
            ...
            this.mEntries.clear();
            this.mTotalSize = 0;
            VolleyLog.m14404d("Cache cleared.", new Object[0]);
        }
    }
        ...
        if (cacheHeader == null) {
            ...
            try {
                ...
                if (TextUtils.equals(str, CacheHeader.readHeader(countingInputStream).key)) {
                    ...
                    VolleyLog.m14404d("%s: key=%s, found=%s", fileForKey.getAbsolutePath(), str, CacheHeader.readHeader(countingInputStream).key);
                    removeEntry(str);
                    countingInputStream.close();
                    entry = null;
                }
            } catch (IOException e) {
                VolleyLog.m14404d("%s: %s", fileForKey.getAbsolutePath(), e.toString());
                remove(str);
                entry = null;
            } catch (Throwable th) {
                countingInputStream.close();
    ...
    public synchronized void initialize() {
        if (this.mRootDirectory.exists()) {
            ...
            VolleyLog.m14405e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
        }
    }
    ...
    public synchronized void put(String str, Entry entry) {
        ...
        try {
            ...
            if (cacheHeader.writeHeader(bufferedOutputStream)) {
                ...
                bufferedOutputStream.close();
                VolleyLog.m14404d("Failed to write header for %s", fileForKey.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e) {
            if (!fileForKey.delete()) {
                VolleyLog.m14404d("Could not clean up file %s", fileForKey.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String str) {
        ...
        removeEntry(str);
        if (!delete) {
            VolleyLog.m14404d("Could not delete cache entry for key=%s, filename=%s", str, getFilenameForKey(str));
        }
    }
}
