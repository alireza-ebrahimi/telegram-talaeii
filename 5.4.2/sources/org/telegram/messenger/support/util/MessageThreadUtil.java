...
import android.support.v4.content.C0427n;
...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C36571();
            AtomicBoolean mBackgroundRunning = new AtomicBoolean(false);
            private final Executor mExecutor = C0427n.m1905a();
            final MessageQueue mQueue = new MessageQueue();

            /* renamed from: org.telegram.messenger.support.util.MessageThreadUtil$2$1 */
            class C36571 implements Runnable {
                C36571() {
                }

                public void run() {
                    while (true) {
                        SyncQueueItem next = C36582.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C36582.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C36582.this.mQueue.removeMessages(2);
                                    C36582.this.mQueue.removeMessages(3);
                        ...
                        C36582.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C36551();
            final MessageQueue mQueue = new MessageQueue();

            /* renamed from: org.telegram.messenger.support.util.MessageThreadUtil$1$1 */
            class C36551 implements Runnable {
                C36551() {
                }

                public void run() {
                    SyncQueueItem next = C36561.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
