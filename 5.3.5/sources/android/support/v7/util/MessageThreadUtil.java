...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C03101();
            ...
            class C03101 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C03111.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C03121();
            ...
            class C03121 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C03132.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C03132.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C03132.this.mQueue.removeMessages(2);
                                    C03132.this.mQueue.removeMessages(3);
                        ...
                        C03132.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
