...
class AdvertisingInfoServiceStrategy implements AdvertisingInfoStrategy {
    ...
    private static final class AdvertisingConnection implements ServiceConnection {
        ...
        public IBinder getBinder() {
            if (this.retrieved) {
                Fabric.getLogger().mo4383e(Fabric.TAG, "getBinder already called");
            }
            this.retrieved = true;
            try {
    ...
    private static final class AdvertisingInterface implements IInterface {
        ...
        public String getId() throws RemoteException {
            ...
            try {
                ...
                Fabric.getLogger().mo4381d(Fabric.TAG, "Could not get parcel from Google Play Service to capture AdvertisingId");
            } finally {
                reply.recycle();
                data.recycle();
            }
        ...
        L_0x002f:
            ...
            r4.mo4381d(r5, r6);	 Catch:{ all -> 0x0044 }
            r3.recycle();
            r0.recycle();
            goto L_0x002c;
        L_0x0044:
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            Fabric.getLogger().mo4381d(Fabric.TAG, "AdvertisingInfoServiceStrategy cannot be called on the main thread");
            return null;
        }
        try {
            ...
            try {
                ...
                Fabric.getLogger().mo4381d(Fabric.TAG, "Could not bind to Google Play Service to capture AdvertisingId");
                return null;
            } catch (Exception e) {
                Fabric.getLogger().mo4395w(Fabric.TAG, "Exception in binding to Google Play Service to capture AdvertisingId", e);
                this.context.unbindService(connection);
                return null;
            } catch (Throwable t) {
                Fabric.getLogger().mo4382d(Fabric.TAG, "Could not bind to Google Play Service to capture AdvertisingId", t);
                return null;
            }
        } catch (NameNotFoundException e2) {
            Fabric.getLogger().mo4381d(Fabric.TAG, "Unable to find Google Play Services package name");
            return null;
        } catch (Exception e3) {
            Fabric.getLogger().mo4382d(Fabric.TAG, "Unable to determine if Google Play Services is available", e3);
            return null;
        }
    }
}
