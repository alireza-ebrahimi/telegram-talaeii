...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C18721();

    /* renamed from: org.telegram.messenger.support.util.DiffUtil$1 */
    static class C18721 implements Comparator<Snake> {
        C18721() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f65x - o2.f65x;
            return cmpX == 0 ? o1.f66y - o2.f66y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f65x != 0 || firstSnake.f66y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f65x + snake.size;
                int endY = snake.f66y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f65x + j;
                    int newItemPos = snake.f66y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f65x + snakeSize;
                int endY = snake.f66y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f65x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f65x + i, 1, this.mCallback.getChangePayload(snake.f65x + i, snake.f66y + i));
                    }
                }
                posOld = snake.f65x;
                posNew = snake.f66y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f65x += range.oldListStart;
                snake.f66y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f65x;
                    left.newListEnd = snake.f66y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f65x - 1;
                    left.newListEnd = snake.f66y;
                } else {
                    left.oldListEnd = snake.f65x;
                    left.newListEnd = snake.f66y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f65x + snake.size;
                    right.newListStart = snake.f66y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f65x + snake.size) + 1;
                    right.newListStart = snake.f66y + snake.size;
                } else {
                    right.oldListStart = snake.f65x + snake.size;
                    right.newListStart = (snake.f66y + snake.size) + 1;
                }
                stack.add(right);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f65x = backward[kOffset + k];
                    outSnake.f66y = outSnake.f65x - k;
            ...
            while (k <= d) {
                ...
                if (checkInFwd || k + delta < (-d) || k + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f65x = backward[kOffset + backwardK];
                    outSnake.f66y = outSnake.f65x - backwardK;
